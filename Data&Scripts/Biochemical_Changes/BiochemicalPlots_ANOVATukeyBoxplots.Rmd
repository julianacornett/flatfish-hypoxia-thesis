...
```{r}
soleHIF <- select(PvetAll, c("DO", "HIF"))
```
...
```{r}
dabHIF <- select(CstigAll, c("DO", "HIF"))
```
...
```{r}
soleLac <- select(PvetAll, c("DO", "Lactate"))
```
...
```{r}
dabLac <- select(CstigAll, c("DO", "Lactate"))
```
...
#Lactate boxplots only

```{r}
jpeg("Lactate_Boxplots.jpg", width = 12, height = 5, units="in", res=1200)

(sole_lac_box | dab_lac_box)
...
```{r}
soleSOD <- select(PvetAll, c("DO", "SOD"))
```
...
```{r}
jpeg("EnglishSole_SODBoxplot.jpg", width = 7, height = 5, units="in", res=1200)
ggplot(soleSOD, aes(DO, SOD)) + 
  geom_boxplot(fill = "lightskyblue1") +
  geom_point(shape=1) +
  theme_pubr() +
  theme(axis.text = element_text(size=14), axis.title = element_text(size=18, face="bold")) +
...
```{r}
dabSOD <- select(CstigAll, c("DO", "SOD"))
```
...
```{r}
jpeg("EnglishSole_BiochemBoxplots.jpg", width = 8.5, height = 11, units="in", res=1200)
...
```{r}
jpeg("SpeckledSanddabs_BiochemBoxplots.jpg", width = 8.5, height = 11, units="in", res=1200)
...
#ANOVAs for Species Comparison Table p-values in Thesis

#dataframe with both species

```{r}
BothBiochem <- rbind(PvetAll, CstigAll)
BothBiochem
```
