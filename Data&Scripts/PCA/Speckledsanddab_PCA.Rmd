...
```{r}
Cstig_vars <- read.csv("../Data/Cstig_AllVars.csv")
```

#remove all rows with NA since PCA doesn't work with missing values

```{r}
Cstig_na <- na.omit(Cstig_vars)
```

#run PCA on all quantitative variables (columns 7-11), excluding ID, species, DO, length, weight, and ambient ventilation rate

```{r}
Cstig.pca <- prcomp(Cstig_na[,c(7:11)], center = TRUE,scale. = TRUE)
summary(Cstig.pca)
```

#plot relative contribution of variables to principle components
#gradient.cols = c("#00AFBB", "#E7B800", "#FC4E07"),
```{r}
#jpeg("Sanddab_varPCA.jpg", width = 7, height = 5, units="in", res=1200)
fviz_pca_var(Cstig.pca, col.var = "contrib", gradient.cols = c("#482677FF", "#20A387FF", "#FDE725FF"), repel = TRUE, legend.title = "Contribution") +
...
#color individuals by DO

```{r}
#jpeg("Sanddab_indPCA.jpg", width = 7, height = 5, units="in", res=1200)
fviz_pca_ind(Cstig.pca, label="none", habillage=Cstig_na$DO,
...
#plot with ellipses & variables arrows

```{r}
#jpeg("SpeckledSanddab_PCA_Biplot.jpg", width = 7, height = 5, units="in", res=1200)
fviz_pca_biplot(Cstig.pca, label="var", repel=TRUE, alpha.ind=0, mean.point=FALSE, col.var = "black", habillage=Cstig_na$DO,
               addEllipses=TRUE, ellipse.level=0.95, legend.title = "DO (mg/L)") + 
...
```{r}
princomp(Cstig_na[,c(7:11)], cor = FALSE, scores = TRUE)
```

#add DO as supplementary qualitative variable ("quali.sup") - DO is column 3

```{r}
res.pca <- PCA(Cstig_na[,c(7:11)], quali.sup = 3, graph = FALSE)
```
...
```{r}
jpeg("SpeckledSanddab_PC1_2.jpg", width = 14, height = 10, units="in", res=1200)
...
dab_pc1_p1 <- ggplot(ind_PC, aes(DO, coordPC1)) +
  ...
  geom_text(aes(x=6.5, y=2.5, label= "R^2 < 0.001~~~p == 0.98"), size=5, parse = TRUE)
...
dab_pc2_p3 <- ggplot(indPC2_noOutlier, aes(DO, coordPC2) ) +
  ...
  xlab("DO (mg/L)") +
  ylab("Individual PC Score (PC2)") +
  geom_text(aes(x=6.5, y=2, label= "R^2 == 0.32~~~p < 0.001 "), size=5, parse = TRUE)
...
dab_pc2_p4 <- ggplot(dab_vPC2, aes(DO, varPC2) ) +
  ...
  xlab("DO (mg/L)") +
  ylab("Variance in Individual PC Score (PC2)") +
  geom_text(aes(x=7, y=0.55, label = "R^2 == 0.54~~~p == 0.31"), size = 5, parse = TRUE)
