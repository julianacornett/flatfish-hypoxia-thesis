...
```{r}
#jpeg("Sole_varPCA.jpg", width = 7, height = 5, units="in", res=1200)
fviz_pca_var(Pvet.pca, col.var = "contrib", gradient.cols = c("#482677FF", "#20A387FF", "#FDE725FF"), 
...
fviz_pca_ind(Pvet.pca, label="none", habillage=Pvet_na$DO, addEllipses=TRUE, 
...
```{r}
jpeg("EnglishSole_PCA_Biplot.jpg", width = 7, height = 5, units="in", res=1200)
fviz_pca_biplot(Pvet.pca, label="var", repel = TRUE, alpha.ind=0, mean.point=FALSE, col.var = "black", habillage=Pvet_na$DO,
...
```{r}
princomp(Pvet_na[,c(7:11)], cor = FALSE, scores = TRUE)
```

#add DO as supplementary qualitative variable ("quali.sup") - DO is column 3

```{r}
res.pca <- PCA(Pvet_na[,c(7:11)], quali.sup = 3, graph = FALSE)
```
...
```{r}
jpeg("EnglishSole_PC1_2.jpg", width = 14, height = 10, units="in", res=1200)
...
sole_pc2_p3 <- ggplot(ind_PC, aes(DO, coordPC2)) +
  ...
  geom_text(aes(x=6.5, y=2.5, label= "R^2 < 0.001~~~p == 0.86"), size=5, parse = TRUE)
